  ...
  navlistPanel(
    ...
    tabPanel("Distribution, Standard of Care",
              ...
              p('Adjuvant treatment is rare in East Africa, so 
                we model no treatment as the standard of care.'),
              br(),
    ...
    tabPanel("Advanced treatment controls",
             h3('Modify treatment distributions for complex scenarios'),
             p('In settings where the standard of care already includes some adjuvant treatment, it may be difficult to specify treatment distributions using the simple controls on the Treatment pages. Modify the numeric vectors below to directly specify, for each stage and ER subgroup, what proportion of the group receives each of the four possible adjuvant treatments.'), 
             tags$b('The proportions for each treatment should be'),
             p('    1. Entered in this order: None, Endocrine, Chemo, and Endocrine+Chemo'), 
             p('    2. Separated by commas'), 
             p('    3. Sum to 1 within each stage-ER group'), 
             br(),
             p('EXAMPLE: The default inputs are set to match the default example, breast cancer in East Africa. In the Colombia example in the manuscript, however, the standard-of-care treatment distribution for advanced ER+ cases 2% none, 10% endocrine, 38% chemo, 50% both. To enter this distribution below, you would type in to the advanced ER+ box: 0.02, 0.10, 0.38, 0.50'),
             br(),
             h4('Standard of Care'),
             h5('ADVANCED STAGE'),
             uiOutput('a0tERpos'),
